...
const hobbies = [
    ...
    },
    {
        ...
        description: "Constantly experimenting with emerging frameworks, languages, and patterns. If it's new and interesting, it gets a weekend.",
        category: 'Learning',
    },
    {
        icon: Book,
        title: 'Technical Reading',
        description: 'Working through programming books, long-form blog posts, and RFCs. Nothing beats reading the manual carefully.',
        category: 'Learning',
    },
    {
        icon: Palette,
        title: 'Interface Design',
        description: 'Sketching layouts in Figma, studying typography, and obsessing over spacing. Building better eyes for better builds.',
        category: 'Creative',
    },
    {
        icon: Coffee,
        title: 'Coffee Brewing',
        description: 'Pour-over, AeroPress, espresso. The ritual matters as much as the result — and the result pairs well with long sessions.',
        category: 'Lifestyle',
    },
    {
        icon: Gamepad2,
        title: 'Strategy Games',
        description: 'Turn-based strategy and puzzles. The skills translate more directly to programming than I care to admit.',
        category: 'Entertainment',
    },
    {
        icon: Dumbbell,
        title: 'Fitness',
        description: 'Regular lifting and conditioning. A reliable body makes everything else — including shipping — feel lighter.',
        category: 'Health',
    },
    {
        icon: Camera,
        title: 'Photography',
        description: 'Composition, light, and patience. A quiet practice that sharpens attention to detail in every other discipline.',
        category: 'Creative',
    },
    {
        icon: Music,
        title: 'Music Production',
        description: 'DAWs, sound design, and the occasional half-finished track. Signal processing and software have more in common than not.',
        category: 'Creative',
    },
    {
        icon: Plane,
        title: 'Travel & Culture',
        description: 'Exploring new places, languages, and food. Distance from the screen reliably returns better ideas to it.',
        category: 'Lifestyle',
    },
];
...
<template>
    <FrontendLayout current-route="/hobby">
        <Head>
            ...
            <meta name="keywords" content="hobbies, interests, personal, developer lifestyle, creative pursuits" />
        ...
        <section v-if="isLoading" class="mx-auto w-full max-w-7xl px-3 py-6 sm:px-6 sm:py-8 lg:px-10">
            <div class="grid w-full grid-cols-2 gap-3 sm:gap-4 md:grid-cols-12 md:gap-5">
                ...
                <Skeleton v-for="i in 6" :key="i" class="col-span-2 h-56 rounded-2xl sm:col-span-1 md:col-span-4" />
            </div>
        </section>

        <section v-else class="relative mx-auto w-full max-w-7xl px-3 py-6 sm:px-6 sm:py-8 lg:px-10" :class="{ 'is-visible': isVisible }">
            <!-- Ambient + grain -->
            ...
            >
                ...
                <h1 class="mt-5 font-serif leading-[0.9] tracking-tight sm:mt-6">
                    <span class="block text-[clamp(2.5rem,9vw,6.5rem)] font-normal text-foreground"> Off </span>
                    <span class="block text-[clamp(2.5rem,9vw,6.5rem)] font-normal text-foreground/80 italic"> -screen. </span>
                </h1>

                <p class="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:mt-6 sm:text-[15px] md:text-base">
                    Beyond the editor — the rituals, obsessions, and practices that refill the well. Each one quietly shapes the way I think, design,
                    and ship code.
                </p>
            </article>
            ...
            <div class="reveal mt-4 sm:mt-5" style="--d: 160ms">
                <div class="card-3d rounded-[1.25rem] border border-border/60 bg-card/60 p-3 backdrop-blur-xl sm:rounded-[1.25rem] sm:p-4">
                    ...
                    <div
                        class="mt-3 flex items-center justify-between font-mono text-[10px] tracking-[0.22em] text-muted-foreground uppercase sm:mt-3"
                    >
                        <span>{{ filteredHobbies.length }} / {{ hobbies.length }} showing</span>
                        <span v-if="selectedCategory !== 'All'" class="hidden sm:inline"> filed under {{ selectedCategory }} </span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 sm:mt-5">
                ...
                >
                    ...
                    <h3 class="mt-5 font-serif text-2xl text-foreground sm:text-3xl">Nothing here yet.</h3>
                    <p class="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">No entries filed under {{ selectedCategory }}.</p>
                    <button
                ...
                <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
                    <article
                        ...
                        class="card-3d hobby-card reveal group relative overflow-hidden rounded-[1.25rem] border border-border/60 bg-card/60 p-5 backdrop-blur-xl sm:p-6"
                        :style="{ '--d': 240 + i * 60 + 'ms' }"
                    >
                        ...
                        <div class="relative z-[1] flex h-full flex-col">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                >
                                    <component :is="hobby.icon" class="h-5 w-5 sm:h-5 sm:w-5" />
                                </span>
