...
class IntermediateTokenizer {
    ...
    processText(token) {
        ...
        let result = null;
        if (this.expressionStartToken != null) {
            const lastToken = this.expressionTokens.at(-1) || this.expressionStartToken;
...
class Parser {
    ...
    processTemplateText(token, templateTokenizerOption) {
        ...
        const count = sortedLastIndexBy(this.tokenizer.tokens, token, (x) => x.range[1]) -
            index;
...
function getScriptSetupModuleCodeBlocks(scriptSetupElement, scriptElement, sfcCode, linesAndColumns, parserOptions) {
    ...
    const scriptSetupOffset = codeBlocks.